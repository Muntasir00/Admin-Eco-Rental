...
const FACILITY_OPTIONS = [
    ...
    { label: "Fast Wifi", value: "Fast Wifi" },
    { label: "LCD Television", value: "LCD Television" },
...
export default function FacilitySheet() {
    ...
    // ২. বর্তমান সিলেক্টেড আইটেমগুলো ট্র্যাক করার জন্য watch
    const watchedFacilityList = form.watch("facilityList");
    ...
    }, [isFacilitySheetOpen, selectedFacility, form, selectedRoomIdForCreate]);
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Button
                                            ...
                                            disabled={fields.length >= FACILITY_OPTIONS.length}
                                        >
                                    ...
                                    {fields.map((field, index) => {
                                        // ৩. ফিল্টারিং লজিক: অন্য রো-তে সিলেক্ট করা অপশনগুলো বাদ দেওয়া
                                        const otherSelectedValues = watchedFacilityList
                                            .map((item, i) => (i !== index ? item.name : null))
                                            .filter(Boolean);

                                        const availableOptions = FACILITY_OPTIONS.filter(
                                            (opt) => !otherSelectedValues.includes(opt.value)
                                        );

                                        return (
                                            <Card key={field.id} className="relative bg-muted/20 py-0">
